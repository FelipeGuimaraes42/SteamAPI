...
def create_csv_file():
    if not os.path.exists('friends_first_level_felipe_pt2_pt2.csv'):
        with open('friends_first_level_felipe_pt2_pt2.csv', 'w', newline='') as csv_file:
...
def extract_csv_info_friends():
    ...
    with open("scraping_connection_error.csv", mode='r', newline='') as file:
        reader = csv.DictReader(file)

        for row in reader:
            # Extract data based on headers and append as a tuple
            friends_ids.append((row['Friend Id'], row['Id']))
...
def save_user_info_csv(friend_id, user_info_parameter):
    if len(user_info_parameter) != 0:
        with open('friends_first_level_felipe_pt2_pt2.csv', 'a', newline='') as csv_file:
            fieldnames = ['Friend Id', 'Id', 'Number of Friends', 'Friends', 'Number of Games', 'Games']
            csv_writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
...
login_to_steam()
create_csv_file()
scraped_ids = extract_csv_info_friends()

for scraped_id in scraped_ids:
    root_id, friend_id = scraped_id
    user_info = scrape_user_info(friend_id)
    save_user_info_csv(root_id, user_info)
